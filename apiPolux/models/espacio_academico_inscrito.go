...
type EspacioAcademicoInscrito struct {
	...
	EstadoEspacioAcademicoInscrito *EstadoEspacioAcademicoInscrito `orm:"column(estado_espacio_academico_inscrito);rel(fk)"`
...
func GetAllEspacioAcademicoInscrito(query map[string]string, fields []string, sortby []string, order []string,
	...
	o := orm.NewOrm()
	qs := o.QueryTable(new(EspacioAcademicoInscrito))
	// query k=v
	for k, v := range query {
